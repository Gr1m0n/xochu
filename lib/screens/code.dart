...
class Code extends StatefulWidget {
  ...
  final String? phone;
  const Code(
      {Key? key, required this.code, required this.email, required this.password, required this.firstName, required this.lastName, required this.phone})
      : super(key: key);
  @override
...
class _CodeState extends State<Code> {
  ...
  Future<void> sendSMS() async {
    var queryParams = {'api_id': 'D196ADE6-4A53-4C7C-2EB6-D991B0F69D9E', 'to': '${widget.phone.toString()}', 'msg': '${widget.code.toString()}', 'json':'1'};
    var headers = {
  ...
  Future<void> registration() async {
    ...
    String queryString = Uri(queryParameters: queryParams).query;
    var requestUrl = "${AppStrings.api}register" + '?' + queryString;
    var response = await http.get(Uri.parse(requestUrl), headers: headers);
    if (response.statusCode == 200) {
      Map mapRes = json.decode(response.body);
